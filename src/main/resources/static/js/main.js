...
function sendMessage(name, str){
    ...
    // JSON.stringify(obj); // JS 객체 -> JSON 

    testSock.send(JSON.stringify(obj)); // JS 객체 -> JSON
...
function getCookie(key){
	// getCookie = "saveId=user01@kh.or.kr; test = 가나다; aaa=100"
    const cookies = document.cookie;

    const cookieList = cookies.split(";").map(cookie => cookie.split("="));
    // split만 이용한 경우 :: ['saveId=user01@kh.or.kr, test = 가나다 , aaa = 1000]
...
if(document.querySelector("input[name='memberEmail']") != null){
    ...
    if(saveId != undefined){ // 쿠키게 저장된 이메일이 있을 때 
